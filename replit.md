...
- `src/game/terrain.ts` — Pure-TS terrain utilities: `getTerrainHeight(x,z)`, `getIslandHeight(x,z)`, `buildTerrainHeightArray()`, `TERRAIN_SIZE/SEGS` constants. Single source of truth for physics and visuals. Imported by Graveyard, PirateIsland, Zombie, Game. **`GENESIS_TERRAIN_SIZE=12000`, `GENESIS_HEIGHT_SCALE=20` (2× island — peak ≈ 2560 m).**
