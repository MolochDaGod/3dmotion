/**
 * data/skills.ts — single source of truth for skill definitions.
 *
 * Owns:
 *   SkillEffectType  — visual effect category
 *   HitShape         — hit geometry shape for damage sweeps
 *   SkillDef         — per-skill definition interface
 *   WEAPON_SKILLS    — 4 skills per weapon, keyed by WeaponMode
 *
 * Imports WeaponMode from data/weapons (NOT from useGameStore) to avoid
 * coupling between a static data module and a Zustand store.
 * Zero Zustand imports — pure static config.
 */

import type { WeaponMode } from "./weapons";

// ─── Effect visual category ───────────────────────────────────────────────────
export type SkillEffectType = "slash" | "blast" | "volley" | "buff";

// ─── Hit geometry shape for damage sweeps ────────────────────────────────────
...
export interface SkillDef {
  ...
  icon:         string;
  description:  string;
  /** AnimKey string — kept as string to avoid circular import with Player.tsx */
  animation:    string;
  timeScale:    number;
